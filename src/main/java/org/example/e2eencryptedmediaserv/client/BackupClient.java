...
@Command(
        ...
        version = "0.1.0",
        description = "E2E Backup CLI Prototype - Encrypts and uploads a file to the server"
)


public class BackupClient implements Callable<Integer> {

    @Parameters(index = "0", description = "Path to the file to download")
    private File file;

    @Option(names = {"--server", "-s"}, required = true, description = "Server URL (e.g. http://localhost:8080)")
    private String serverUrl;

    @Option(names = {"--key", "-k"}, required = true, description = "Base64-encoded 256-bit encryption key")
    private String keyBase64;

    @Option(names = {"--token", "-t"}, defaultValue = "dummy-token", description = "Bearer authorization token (if required)")
    private String token;

    @Option(names = {"--client-id", "-c"}, defaultValue = "alice", description = "Client ID")
